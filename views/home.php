<div class='hero-unit'>
	<h2>Try to make my world simple and stupid...</h2>
</div>

<div id='functions' class='row-fluid'>
	<div class='span4'>
		<h3>Blogs</h3>
		<p class='alert alert-info'>
			<strong class='label label-info'>BLOG</strong> &nbsp;&nbsp;record my workings.
		</p>
		<a href='<?=l("?func=blogs")?>' class=' btn'>View details »</a>
	</div>
	<div class='span4'>
		<h3>Wikis</h3>
		<p class='alert alert-info'>
			<strong class='label label-info'>WIKI</strong> &nbsp;&nbsp;contains my knowledge.
		</p>
		<a href='<?=l("?func=wikis")?>' class=' btn'>View details »</a>
	</div>
	<div class='span4'>
		<h3>Utils</h3>
		<p class='alert alert-info'>
			<strong class='label label-info'>UTIL</strong> &nbsp;&nbsp;special cool.
		</p>
		<a href='<?=l("?func=utils")?>' class=' btn'>View details »</a>
	</div>
</div>
