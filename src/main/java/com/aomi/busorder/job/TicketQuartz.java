        ...
        for (int j = 0; j < dusers.size(); j++) {
          if (dusers.get(j).getUID().equals(users.get(i).getUid())) {
            for (int k = 0; k < departmentList1.size(); k++) {
              if (dusers.get(j).getDID()
                  .equals(departmentList1.get(k).getDID())) {
                ...
                if (departmentList1.get(k).getLevel() == 3) {
                  for (int g = 0; g < departmentList1.size(); g++) {
                    if (departmentList1.get(g).getDID()
                        .equals(departmentList1.get(k).getParentID())) {
                      user.setDepartment(
                          departmentList1.get(g).getDepartmentName());
                    }
                  }
                }
              }
