...
public class LdapService {
    ...
    try {
      ...
      while (answer.hasMoreElements()) {
        ...
        if (Attrs != null) {
          ...
          if ((i == 0 && !department.getDID().equals("BusinessDocumentAdmin"))
              && (!department.getDID().equals("领导"))
              && !department.getDID().substring(0, 1).equalsIgnoreCase("G")
              && !department.getDID().substring(0, 1).equalsIgnoreCase("D")
              && !department.getDID().substring(0, 1).equalsIgnoreCase("T")
              && !(department.getDID().length() == 4)
              && department.getDepartmentName().indexOf("(原)") == -1) {
            ...
            if (department.getLevel() == 1 || department.getLevel() == 2) {
              ...
              // DUser
              // du=dUserDao.findByUIDAndDID(duser.getUID(),duser.getDID());
              // if(du==null){
              // dUserDao.save(duser);
              // }
              // }
  ...
  public List<Usera> getUserInfo() {
    ...
    int totalResults = 0;
    String[] returnedAtts = { "uid", "birthday", "degree", "displayName",
        "gender", "mail", "title", "isSystem", "userpassword" }; // 定制返回属性
    LdapQuery ldapQuery = new LdapQuery();
    ldapQuery.setSearchFilter(searchFilter);
    ldapQuery.setSearchBase(searchBase);
    ldapQuery.setReturnedAtts(returnedAtts);
    ...
    try {
      ...
      while (answer.hasMoreElements()) {
        ...
        if (Attrs != null) {
          ...
          // (isNumeric(user.getUid())) ||(user.getUid().indexOf("gk_")>-1))) {
          ...
          // userDao.save(user);
          // URole uRole =
          // uRoleDao.findByRidAndUid(role.getRid(),user.getUid());
          // if(uRole==null){
          // uRole = new URole();
          // uRole.setRid("0001");
