...
module.exports = Vue.extend({
    ...
    methods:{
        ...
        getUsers:function(param){
            var self = this;
            this.loading = true;
            Service.getUsers({page:self.page,limit:self.limit,type:self.user.type,email:this.query.email?this.query.email:undefined,admin:this.query.admin<0?undefined:this.query.admin,name:this.query.name?this.query.name:undefined},function (rep) {
                self.loading = false;
                if(rep.Code == 0){
                    self.users = rep.Response.lists;
                    self.count = rep.Response.count;
