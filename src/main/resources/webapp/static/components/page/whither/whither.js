   ...
   methods:{
      render: function () {
         ...
         lst.each(function (index,target) {
            for(var i in self.whithers){
               if($(target).data("group") == self.whithers[i].group){
                  ...
                  for(var l in ws){
                     $(target).after(' <li data-value='+ws[l].id+' data-name='+ws[l].name+' data-tags='+ws[l].tag+' class="mui-table-view-cell mui-indexed-list-item">'+ws[l].name+'</li>');
                  }
               }
            }
         });
         ...
         $list.delegate(".mui-indexed-list-item","click", function (e) {
            var v = $(e.target).data("value"),name = $(e.target).data("name");
            self.search.target = name;
            self.search.value = v;
