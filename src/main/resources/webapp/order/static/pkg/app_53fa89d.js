...
function getPackage(c){
    $.get(prefix +"/data/packs.json",co(c));
}
...
module.exports = Vue.extend({
    ...
    methods:{
        ...
        checkDh: function (config) {
            ...
            begin.setDate(begin.getDate()+parseInt(config.begin.week) - w);
...
store = {
    ...
    getOrders: function () {
        ...
        Service.getOrders({start:start,end:end},function (rep) {
            ...
            self.state.orders = self._trans(list);
        });
    },
    _trans: function (rep) {
        var list = [];
        if(rep.length>0){
            list.push(rep[0]);
            for (var i = 1,j=0; i < rep.length; i++) {
                var obj = rep[i];
                if(this.isSame(rep[j],rep[i])){
                    rep[j]._id += "_"+rep[i]._id;
                    rep[j].table.no += ","+rep[i].table.no;
                }else{
                    list.push(rep[i]);
                    j = i;
                }
    ...
    isSame: function (a,b) {
        return a.mdate == b.mdate && a.number == b.number && a.mtime == b.mtime;
    },
    getGoods: function () {
        var start = (new Date()).Format("yyyy-MM-dd");
        var end =  ((new Date()).Format("yyyy")*1+1)+(new Date()).Format("-MM-dd");
...
router.map({
    ...
    "history/detail/:id":{
        component: function (resolve) {
            require.async(["components/page/history/detail/detail"], function (p) {
                resolve(p);
            });
        }
    }
...
module.exports = Vue.extend({
    template:"<div class=\"mui-content\" id=\"map\" :style=\"{height:height+'px'}\">\n</div>\n<table class=\"table-footbar\" v-show=\"selectTable.length>0\" transition=\"slideInDown\">\n  <tr>\n      <td >\n          已选:\n          <span class=\"table-no\" v-for=\"t in selectTable\">{{t.no<10?('0'+t.no):t.no}}</span>\n      </td>\n      <td >\n          <a class=\"footbar-btn\" href=\"javascript:;\" @click=\"onSubmitAll\">提交</a>\n      </td>\n  </tr>\n</table>",
    data: function () {
        return {
    ...
    methods:{
        ...
        renderHot: function (data) {
            ...
            feature.on("click", function () {
                ...
                if(this.options.data.user){
                    ...
                    var color = this.options.fillColor == "#fef1c9"?"blue":"#fef1c9";
                    if(color){
                        this.setStyle({fillColor:color});
                    }

                    var has = false;
                    for (var i = 0; i < self.selectTable.length; i++) {
                        var obj = self.selectTable[i];
                        if(obj._id == self.table._id){
                            self.selectTable.$remove(self.table);
                            has = true;
                        }
                    }

                    if(!has&&self.selectTable.length<6){
                        self.selectTable.push(self.table);
                    }else{
                        this.setStyle({fillColor:"#fef1c9"});
                    }
                    this.redraw();
        ...
        getDialogInfo: function () {
            return this.$interpolate("<table class='table-info'>" +
                ...
                "<tr><td>桌位:</td><td>{{selectTableStr}}</td></tr>" +
                "</table>");
        },
        order: function () {
            var self = this;
            ...
            var ids = [];
            this.selectTable.forEach(function (s) {
                ids.push(s._id);
            });
            Service.orderTable(ids.join(","),JSON.stringify({
                ...
                setTimeout(function () {
                    ...
                    store.getOrders();
                    self.renderTable();
                    self.$router.replace("/food/"+ids.join("_")+"/list/all");
                },1000);
            })
        },
...
module.exports = Vue.extend({
    ...
    methods:{
        ...
        submit: function () {
            ...
            var self = this;
            this._putOrder({menu: m,_id:this.$route.params.oid.replace(/_/g,","),remark:this.remark,number:this.number}, function () {
                self.ok.show = true;
            });
        },
        ...
        onCancel: function () {
            ...
            layer.open({
                ...
                btn:["确定","取消"],
                yes: function () {
                    Service.delOrder(self.$route.params.oid.replace(/_/g,","), function (rep) {
    ...
    route:{
        data: function (r) {
            var t = r.to.params.oid.split("_");
            this.oid = t.length>0?t[0]:r.to.params.oid;
            this.selectType = this.types.length>0?this.types[0].key:"";
            if(this.packages.length>0){
                this.selectType = "package";
            }
...
module.exports = Vue.extend({
    template:"<div class=\"page-history\">\n    <div class=\"weui_cells\" style=\"margin-top: 0\">\n\n        <div class=\"weui_cell weui_cell_select weui_select_after\">\n            <div class=\"weui_cell_hd\">\n                <label for=\"\" class=\"weui_label\">查询时间</label>\n            </div>\n            <div class=\"weui_cell_bd weui_cell_primary\">\n                <select class=\"weui_select\" name=\"select2\" style=\"padding: 0;margin-bottom: 0;\" v-model=\"month\">\n                    <option value=\"{{m}}\" v-for=\"m in months\">{{m}}</option>\n                </select>\n            </div>\n        </div>\n    </div>\n\n\n    <div class=\"mui-content\"  >\n\n        <ul class=\"mui-table-view\"  v-if=\"app=='food'\">\n            <li style=\"text-align: center\" v-show=\"count ==0\">\n                无该月历史订单\n            </li>\n            <li class=\"mui-table-view-cell mui-media\" v-for=\"l in list\" v-if=\"l.state>0\">\n                <a class=\"mui-navigate-right\" v-link=\"{path:'/history/detail/'+l._id}\" >\n                    <div class=\"mui-media-body\">\n                        {{l.mdate}} {{l.mtime==\"lunch\"?\"午餐\":\"晚餐\"}}\n                        <p class='mui-ellipsis'>{{l.table.no}}号桌({{l.table.type==0?\"包间\":\"大厅\"}})  {{l.number}}人</p>\n                    </div>\n                </a>\n            </li>\n        </ul>\n        <ul class=\"mui-table-view\"  v-if=\"app=='goods'\">\n            <li style=\"text-align: center\" v-show=\"count ==0\">\n                无该月历史订单\n            </li>\n            <li class=\"mui-table-view-cell mui-media\" v-for=\"l in list\" v-if=\"l.state>0\">\n                <a class=\"mui-navigate-right\" v-link=\"{path:'/history/detail/'+l._id}\" >\n                    <div class=\"mui-media-body\">\n                      取货日期  {{l.mdate}}\n                    </div>\n                </a>\n            </li>\n        </ul>\n    </div>\n\n    <loading :show=\"loading\" text=\"查询中...\" ></loading>\n</div>",
    data: function () {
        return {
            months:[],
            month:"",
    ...
    methods:{
        getMonth: function () {
            var d = new Date();
            for (var i = 0; i < 6; i++) {
                this.months.push(d.Format("yyyy-MM"));
    ...
    route:{
      data: function () {
          this.app = this.$route.params.type;
      }
    },
    ready: function () {
        this.getMonth();
...
;/*!/components/page/history/detail/detail.js*/
define('components/page/history/detail/detail', function(require, exports, module) {
...
module.exports = Vue.extend({
    template:"<div class=\"page-history-detail\">\n    <div v-if=\"app=='food'\">\n        <div class=\"weui_cells\">\n            <div class=\"weui_cell\">\n                <div class=\"weui_cell_bd weui_cell_primary\">\n                    <p style=\"margin-bottom: 0;font-size: 17px;color: #000;\">就餐人数</p>\n                </div>\n                <div class=\"weui_cell_ft\">{{order.number}}人</div>\n            </div>\n            <div class=\"weui_cell\">\n                <div class=\"weui_cell_bd weui_cell_primary\">\n                    <p style=\"margin-bottom: 0;font-size: 17px;color: #000;\">就餐日期</p>\n                </div>\n                <div class=\"weui_cell_ft\">{{order.mdate}}</div>\n            </div>\n            <div class=\"weui_cell\">\n                <div class=\"weui_cell_bd weui_cell_primary\">\n                    <p style=\"margin-bottom: 0;font-size: 17px;color: #000;\">就餐餐次</p>\n                </div>\n                <div class=\"weui_cell_ft\">{{order.mtime=='dinner'?'晚餐':'中餐'}}</div>\n            </div>\n            <div class=\"weui_cell\">\n                <div class=\"weui_cell_bd weui_cell_primary\">\n                    <p style=\"margin-bottom: 0;font-size: 17px;color: #000;\">就餐桌号</p>\n                </div>\n                <div class=\"weui_cell_ft\">{{order.table.no}}号桌</div>\n            </div>\n        </div>\n\n        <ul class=\"mui-table-view\">\n            <li class=\"mui-table-view-cell mui-media\" v-for=\"l in order.menu\" v-show=\"l.number>0\" >\n                <template v-for=\"p in l.picture\" >\n                    <img class=\"mui-media-object mui-pull-left  menu-img\" :src=\"'/data/image/'+p+'/60/60.json'\" data-preview-src=\"{{'/data/image/'+p+'/1000/1000.json'}}\" data-preview-group=\"{{'o_'+l._id}}\"  v-if=\"$index==0\">\n                    <img class=\"mui-media-object mui-pull-left\" style=\"display: none\" src=\"\" data-preview-src=\"{{'/data/image/'+p+'/1000/1000.json'}}\"  data-preview-group=\"{{'o_'+l._id}}\"    v-if=\"$index>0\">\n                </template>\n                <div class=\"mui-media-body\">\n                    {{l.name}} <span class=\"dz\"><img src=\"/order/static/images/dz.png\" alt=\"\">{{l.star}}</span><span class=\"mui-pull-right\"  v-show=\"mode!='all'\">{{l.number}} {{l.unit}}</span>\n                    <p class='mui-ellipsis-2'>{{l.cates.join(\",\")}}</p>\n                </div>\n                <p class=\"f-info\">\n                    <span class=\"f-price\">¥{{l.price}}</span>/{{l.unit}}\n\n                    <span class=\"mui-numbox mui-pull-right\" v-show=\"mode=='all'\" v-if=\"!l.overtime\">\n                        <button class=\"mui-btn mui-numbox-btn-minus\" type=\"button\" :disabled=\"!l.number\" @click=\"onMinusNum(l)\" >-</button>\n                        <input class=\"mui-numbox-input\" type=\"number\" :value=\"l.number\" />\n                        <button class=\"mui-btn mui-numbox-btn-plus\" type=\"button\" @click=\"onPlusNum(l,$event)\">+</button>\n                    </span>\n                    <span class=\"mui-pull-right mui-badge\" v-show=\"l.overtime\">已过退订时间</span>\n                </p>\n                <p class=\"mui-ellipsis\">\n                    <input type=\"text\" v-show=\"mode=='all'\"  v-model=\"l.remark\" placeholder=\"请填写备注信息,例如口味,要求。\" style=\"margin-bottom: 0;margin-top: 5px;border: 1px solid rgba(0, 0, 0, 0.1)\">\n                    <span v-show=\"mode!='all'\">{{l.remark}}</span>\n                </p>\n            </li>\n\n            <li class=\"mui-table-view-divider\" v-show=\"count==0\"  @click=\"onChangeView('list')\"  >\n                请先点菜\n            </li>\n\n        </ul>\n\n    </div>\n    <div v-else>\n        <div class=\"weui_cells\">\n            <div class=\"weui_cell\">\n                <div class=\"weui_cell_bd weui_cell_primary\">\n                    <p style=\"margin-bottom: 0;font-size: 17px;color: #000;\">取货日期</p>\n                </div>\n                <div class=\"weui_cell_ft\">{{order.mdate}}</div>\n            </div>\n\n        </div>\n        <ul class=\"mui-table-view\">\n            <li class=\"mui-table-view-cell mui-media\" v-for=\"l in order.items\" v-show=\"l.number>0\" >\n                <template v-for=\"p in l.picture\" >\n                    <img class=\"mui-media-object mui-pull-left  menu-img\" :src=\"'/data/image/'+p+'/60/60.json'\" data-preview-src=\"{{'/data/image/'+p+'/1000/1000.json'}}\" data-preview-group=\"{{'o_'+l._id}}\"  v-if=\"$index==0\">\n                    <img class=\"mui-media-object mui-pull-left\" style=\"display: none\" src=\"\" data-preview-src=\"{{'/data/image/'+p+'/1000/1000.json'}}\"  data-preview-group=\"{{'o_'+l._id}}\"    v-if=\"$index>0\">\n                </template>\n                <div class=\"mui-media-body\">\n                    {{l.name}} <span class=\"dz\"><img src=\"/order/static/images/dz.png\" alt=\"\">{{l.star}}</span><span class=\"mui-pull-right\"  v-show=\"mode!='all'\">{{l.number}} {{l.unit}}</span>\n                    <p class='mui-ellipsis-2'>{{l.cates.join(\",\")}}</p>\n                </div>\n                <p class=\"f-info\">\n                    <span class=\"f-price\">¥{{l.price}}</span>/{{l.unit}}\n\n                    <span class=\"mui-numbox mui-pull-right\" v-show=\"mode=='all'\" v-if=\"!l.overtime\">\n                        <button class=\"mui-btn mui-numbox-btn-minus\" type=\"button\" :disabled=\"!l.number\" @click=\"onMinusNum(l)\" >-</button>\n                        <input class=\"mui-numbox-input\" type=\"number\" :value=\"l.number\" />\n                        <button class=\"mui-btn mui-numbox-btn-plus\" type=\"button\" @click=\"onPlusNum(l,$event)\">+</button>\n                    </span>\n                    <span class=\"mui-pull-right mui-badge\" v-show=\"l.overtime\">已过退订时间</span>\n                </p>\n                <p class=\"mui-ellipsis\">\n                    <input type=\"text\" v-show=\"mode=='all'\"  v-model=\"l.remark\" placeholder=\"请填写备注信息,例如口味,要求。\" style=\"margin-bottom: 0;margin-top: 5px;border: 1px solid rgba(0, 0, 0, 0.1)\">\n                    <span v-show=\"mode!='all'\">{{l.remark}}</span>\n                </p>\n            </li>\n\n            <li class=\"mui-table-view-divider\" v-show=\"count==0\"  @click=\"onChangeView('list')\"  >\n                请先点菜\n            </li>\n\n        </ul>\n    </div>\n    <loading :show=\"loading\" text=\"加载中...\" ></loading>\n</div>",
    data: function () {
        return {
            order:{
                menu:[],
