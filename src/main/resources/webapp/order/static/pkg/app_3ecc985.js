    ...
    methods:{
        ...
        renderHot: function (data) {
            ...
            if(data.table.position.type == "rectangle"){
                feature = new L.rectangle(data.table.position.latlngs,{weight:1,fillColor:this.getStateColor(data.state),fillOpacity:data.state==1?0.7:0.5,data:data}).addTo(this.featureLayer);

            }else{
                feature = new L.circle(data.table.position.latlng,data.table.position.radius,{weight:1,fillColor:this.getStateColor(data.state),fillOpacity:data.state==1?0.7:0.5,data:data}).addTo(this.featureLayer);
        ...
        getDialogInfo: function () {
            return this.$interpolate("<table class='table-info'>" +
                ...
                "<tr><td>桌位:</td><td>{{table.type==0?'包厢':'大厅'}}{{table.no}}号</td></tr>" +
                "</table>");
        },
        order: function () {
            var self = this;
            Service.orderTable(this.table._id,JSON.stringify({
                ...
                setTimeout(function () {
                    layer.closeAll();
                    store.getOrders();
                    self.$router.replace("/food/"+rep._id+"/list/nomenu");
                },1000);
            })
        }
    },
